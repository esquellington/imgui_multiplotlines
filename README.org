...
comments in imgui_multiplotlines.h for specific details.

The user-owned Params struct can be either non-persistent, rebuilt
before each MultiPlotLines() call, or persistent across calls.
...
#+BEGIN_SRC cpp
  ...
  MultiPlotLines( label, get_value, data, num_values, num_channels, &params );
#+END_SRC
...
most available features, and ImGui::MultiPlotLines_Params struct comments.
